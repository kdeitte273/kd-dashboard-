...
export default async (req) => {
  ...
  console.log(`[KD Scanner v5] Total unique contracts found: ${unique.length}`);
  ...
  try {
    ...
    console.log(`[KD Scanner v5] Saved ${unique.length} contracts to Netlify Blobs successfully`);
    const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
    await store.setJSON(`scan-${timestamp}`, saveData);
    console.log(`[KD Scanner v5] History copy saved`);
  } catch (blobErr) {
    console.error("[KD Scanner v5] Blob save error:", blobErr.message);
  }
};
