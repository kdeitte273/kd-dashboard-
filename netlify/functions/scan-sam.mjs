export default async (req) => {
  ...
  const titleKeywords = ["housing","lodging","hotel","motel","furnished","temporary housing","seasonal housing","fire housing","crew housing","extended stay","apartment","transitional","cottage","guest house"];
  ...
  console.log(`[KD Scanner v6] Total unique contracts found: ${unique.length}`);
  console.log(`[KD] Errors: ${errors.length > 0 ? errors.map(e => `${e.label}: ${e.status||e.error}`).join(" | ") : "None"}`);
  console.log(`[KD Scanner v6] Sample contract titles:`);
  unique.slice(0, 5).forEach((c, i) => {
    console.log(`  ${i+1}. ${c.title || c.solicitationTitle || "No title"} — ${c.postedDate || ""}`);
  });
  console.log("[KD Scanner v6] Complete. No storage errors.");
};
